...
(function($)
{
    ...
    {
        var content = options._resourceBundle.__('%count% new tweet', '%count% new tweets', count, {
            '%count%' : count
        });
    ...
    defaultTweetTimestampFormatter = function(timeStamp, options)
    {
        ...
        if (diff < 60)
        {
            tweetTimestamp += options._resourceBundle.__('%secs% second ago', '%secs% seconds ago', diff, {
                '%secs%': diff
            });
        }
        else if (diff < 3600)
        {
            ...
            tweetTimestamp += options._resourceBundle.__('%mins% minute ago', '%mins% minutes ago', t, {
                '%mins%': t
            });
        }
        else if (diff < 86400)
        {
            ...
            tweetTimestamp += options._resourceBundle.__('%hours% hour ago', '%hours% hours ago', t, {
                '%hours%': t
            });
        }
        else
        {
            ...
            tweetTimestamp += ' (' + options._resourceBundle.__('%days% day ago', '%days% days ago', t, {
                '%days%': t
            }) + ')';
        }
    ...
    defaultTweetRetweeterDecorator = function(tweet, options)
    {
        ...
        if (tweet.retweeted_status)
        {
            ...
            var rtcount = options._resourceBundle.__(' and %rtc% other', ' and %rtc% others', rtc, {
                '%rtc%': rtc
            });
    ...
    getFeedUrl = function(options, flPaging)
    {
        ...
        else if (options.list)
        {
            ...
            {
                url += '//api.twitter.com/1/favorites/' + options.username + '.json?count=100';
            }
            else
            {
                url += '//api.twitter.com/1/' + options.username + '/lists/' + options.list + '/statuses.json?per_page=100';
            }
        }
        else
        {
            url += '//api.twitter.com/1/statuses/user_timeline.json?screen_name=' + options.username + '&count=100';
        ...
        url += '&callback=?';
        url = '/twitter.php?url='+ encodeURIComponent(url) +'&callback=?';
        //console.log(url);
        return url;
    };
    ...
    setupOptions = function(options)
    {
        ...
        options.count = validateRange(options.count, 0, options.searchParams ? 100 : 100);
    ...
    addHovercards = function(options)
    {
        if (isAnywherePresent())
        {
            ...
            {
                T(options._baseSelector + ' .jta-tweet-list').hovercards({
    ...
    startAutorefresh = function(options)
    {
        if (options._tweetFeedConfig.autorefresh.mode != 'none' &&
            ...
            window.setTimeout(function() {
                processAutorefresh(options);
            }, options._tweetFeedConfig.autorefresh.interval * 1000);
        }
    };
    ...
    startTimestampRefresh = function(options)
    {
        if (
            ...
            {
            window.setTimeout(function() {
                processTimestampRefresh(options);
            }, options._tweetFeedConfig.showTimestamp.refreshInterval * 1000);
        }
    };
